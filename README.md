...
This project implements a multi-target monitoring system that checks a list of specified hosts (IP addresses or URLs) to check their availability. It uses a Python script to perform the monitoring,[...]
...
1.  **Trigger**: Monitoring is triggered by a GitHub Actions workflow defined in `.github/workflows/run-monitor.yml`. This runs automatically on any push to the `main` branch, on an hourly schedul[...]
...
6.  **Output Generation**:
    ...
    *   A static `docs/index.html` file is generated, displaying the latest status and the historical bar chart for each target. A static footer with a link to the Salesforce status page is also i[...]
7.  **Deployment**: The GitHub Actions workflow uses the `stefanzweifel/git-auto-commit-action` to automatically commit the updated contents of the `docs` directory directly back to the `main` bra[...]

## Configuration

To add or change a target, simply edit the `monitoring_targets.txt` file and commit the change to the `main` branch. The order of the services on the status page is determined by the order of the [...]
...
1. Go to https://share.streamlit.io and connect your GitHub account.
2. Select the repository `tsyUF/Monitor` and the branch `Main`.
3. Set the app file path to `streamlit_app/app.py` and deploy.

Make sure `requirements.txt` lists `streamlit` (add a line: `streamlit`) so the Cloud deploy installs the dependency.

Notes:

* The Streamlit app reads `docs/data/results.json` and `docs/chart_*.png` generated by the existing monitor script. The GitHub Actions workflow commits those files back to the repository, so the deployed app will show the latest committed data.
* If you prefer the app to always reflect live (not-yet-committed) results, consider running the monitor on the same host as Streamlit or posting results to a shared endpoint (S3/HTTP) that the app queries.
